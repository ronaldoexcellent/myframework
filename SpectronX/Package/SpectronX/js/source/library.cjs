...
$classes = ["spectron-Console", 's-modal'];
...
$con2.innerHTML = `
    <spectron_js_app_timer class="s-container s-block s-dropdown-click s-border-bottom s-padding">
        <span class="spectron_js_app_timer_RUN s-center" style="font-weight:bold">App Timer: Weeks - 0, Days - 0, Hours - 0, Minutes - 0, Seconds - 0</span>
        <span class="s-dropdown-content s-container s-border spectron_js_app_timer_info s-padding">
            &times; Calculates how long you've stayed on this app.
        </span>
    </spectron_js_app_timer>
    <ul class="s-ul"></ul><br>
    <button class="s-btn s-hover-red" onclick="app.messages.hide()">&times; Close</button>
`;
document.querySelector('.spectron-Console').appendChild($con2);
document.querySelector('spectron_js_app_timer').addEventListener('click', () => toggleClass($('.spectron_js_app_timer_info'), "s-show"));

function spectron_js_app_timer_runner() {
    $secs = -1, $mins = 0, $hrs = 0, $days = 0, $weeks = 0;
    setInterval(()=>{
        $secs++;
        $temp = document.querySelectorAll('.spectron_js_app_timer_RUN');
...
spectron_js_app_timer_runner();

app.messages = {
    "show" : () => $('.spectron-Console').style.display = "block",
    "hide" : () => $('.spectron-Console').style.display = "none"
};

app.timer = {
   clone : ($targ) => $targ.appendChild(document.querySelector('spectron_js_app_timer').cloneNode(true))
};

app.log = (msg) => {
    document.querySelector('.spectron-Console div ul').innerHTML += `<li class="s-hover-blue">${msg}</li>`;
};
...
const changeAppMode = ($mode) => {
    ...
    if ($mode == 'dark') {
        ($('.spectron-Console .s-black')) ? null : addClass($('.spectron-Console div'), "s-black"), removeClass($('.spectron-Console div'), "s-white");
        ...
        ($('.spectron-Console .s-white')) ? null : addClass($('.spectron-Console div'), "s-white"), removeClass($('.spectron-Console div'), "s-black");
...
app.log('<li class="s-green s-italic" style="margin-top:-20px">Spectron connected successfully!</li>');
console.timeEnd("Load Time");
console.log("Spectron connected successfully!");
// End