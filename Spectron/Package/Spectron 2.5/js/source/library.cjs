...
const msgsForApp = () => {
    return `<div class+"spectron-Console s-container s-display-topmiddle s-white"></div>`;
}; const backgroundCount = () => {
    // Conection Test
    $msg = 'Spectron connected successfully!'
    pushAfter($('.spectron-Console h2'), `<br><span style="background:green; color: white;">${$msg}</span>`);

    // App Time
    pushAfter($('.spectron-Console'), '<section class="spectron_js_app_timer" style="padding: 1%; display: block; width: 100%;"></section>');
    $secs = -1;
    $mins = 0;
    $hrs = 0;
    $days = 0;
    ...
    setInterval(()=>{
        $secs++;
        html($('.spectron_js_app_timer'), `<h2>App Time:</h2> <h3>You have spent the following time on this app. <br> <br> Weeks - ${$weeks}</h3> <h3>Days - ${$days}</h3> <h3>Hours - ${$hrs}</h3> <h3>Minutes - ${$mins}</h3> <h3>Seconds - ${$secs}</h3> <hr />`);
        if ($secs == 59) {
            $secs = -1;
            $mins++;
        } if ($mins == 59) {
...
app.messages = () => {
    $('.spectron-Console').style.display = "block";
};
app.footnote = () => {
...
console.log('Loading Spectron Framework/Library... Version 2.5');
...
widget.setAttribute('class', 'spectron-Console');
html(widget, `<h2 style='padding: 1%; line-height: 2;'> Messages: <button onclick="$('.spectron-Console').style.display='none'">Close</button> </h2> <hr />`);
...
// Do not add, remove, modify or change anything in this s-s, except you have a full understanding of how it works.
...
const changeAppMode = ($mode) => {
    $('.spectron-Console').style.background = "rgb(0, 0, 0)";
    $('.spectron-Console').style.color = "white";
    ...
    if ($mode == 'dark') {
        ...
        $('.spectron-Console').style.background = "rgb(255, 255, 255)";
        $('.spectron-Console').style.color = "black";
